{ stdenv, buildGoPackage, go, pkgs, fetchFromGitHub, androidPkgs, openjdk, xcodeWrapper }:

with stdenv;

let
   gomobile = pkgs.callPackage ./gomobile { inherit (androidPkgs) platform-tools; inherit xcodeWrapper; };
